...
class ProductService
{
    ...
    public function getProductsList(array $ids = []) : array
    {
        ...
        $query = "SELECT b.id book_id, b.title, a.name author, g.name genre, b.cost, b.url FROM book b
...
//    ORDER BY b.title LIMIT $offset,8
    ...
    {
        $query = "SELECT b.id book_id, b.title, a.name author, g.name genre, g.id genre_id, b.cost, b.url FROM book b
    left join author a ON a.id = b.author_id
    left join genre g ON g.id = b.genre_id
    where b.url = ?
    ";
